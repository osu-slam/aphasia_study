...
% First nineish rows are training. 
stim_index = label((10:end),4); 
categories = label(1,:);
correct = num(9:end,3);
%correct_trial = num(1:14,3);
total_trial_number = 48; % This should be 48, not 40. MH
response_time = num(9:end,5);

%find total accuracy
acc_total = sum(correct)/total_trial_number;
...
counters = [OC_counter, SC_counter, OD_counter,SD_counter];

for run=1:runs
    
    for jj = 1:total_trial_number
        if contains(stim_index{jj}, conditions{run, 1}) % Better syntax. MH
%             any(~cellfun('isempty',strfind(stim_index(j),conditions(run,1)))) == 1 
            counters(run) = correct(jj) + counters(run);
        elseif contains(stim_index{jj}, conditions{run, 2}) % Better syntax. MH
%             any(~cellfun('isempty',strfind(stim_index(j), conditions(run,2)))) == 1
            counters(run) = correct(jj) + counters(run);
      	end 
    end 
end
...
trials_per_run = total_trial_number/4; % This becomes 12, not 10
...
for run=1:runs
    for jj = 1:total_trial_number
          if contains(stim_index{jj}, conditions{run, 1})
%               any(~cellfun('isempty',strfind(stim_index(jj),conditions(run,1)))) == 1 
              if correct(jj) == 1    
                  rt_counters(run) = response_time(jj) + rt_counters(run);
              end
            
          elseif contains(stim_index{jj}, conditions{run, 2})
%               any(~cellfun('isempty',strfind(stim_index(jj), conditions(run,2)))) == 1
              if correct(jj) == 1 
                  rt_counters(run) = response_time(jj) + rt_counters(run);
              end
          end 
...
%               any(~cellfun('isempty',strfind(stim_index(jj),conditions(run,1)))) == 1 
              ...
              if correct(jj) == 0    
                  wrt_counters(run) = response_time(jj) + wrt_counters(run);
              end
...
%find avg RT for each condition 
wOC_rt = wrt_OC_counter / (trials_per_run - OC_counter) ;
wSC_rt = wrt_SC_counter / (trials_per_run - SC_counter);
wOD_rt = wrt_OD_counter / (trials_per_run - OD_counter);
wSD_rt = wrt_SD_counter / (trials_per_run - SD_counter); 
...
wrong_rt = ((wOC_rt + wSC_rt + wOD_rt + wSD_rt) / (total_trial_number - sum(correct))); 

%find ACC/run 
run_divider = {(1:12), (13:24), (25:36), (37:48)}; % Updated from 1:10, 11:20, 31:30, 31:40. MH
...
% Graph RT 
X=[1 2 3 4];
Y=[OC_rt, SC_rt, OD_rt, SD_rt];
SE=[std_RT_OC std_RT_SC std_RT_OD std_RT_SD];

bar(X,Y);

set(gca,'XTickLabel',['OC'; 'SC'; 'OD'; 'SD'])
...
figure()
X1=[1 2 3 4];
Y1=[OC_acc, SC_acc, OD_acc, SD_acc] *100;
